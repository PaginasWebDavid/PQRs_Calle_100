...
export function ReportesView() {
  ...
  async function exportExcel() {
    ...
    const resumenData = [
      ...
      [],
      ["RESUMEN GENERAL"],
      ["Total PQRS", data.resumen.total],
      ["Peticiones", data.resumen.byTipo.peticion],
      ["Quejas", data.resumen.byTipo.queja],
      ["Reclamos", data.resumen.byTipo.reclamo],
      ["Sugerencias", data.resumen.byTipo.sugerencia],
      [],
      ["ESTADO ACTUAL"],
      ["En espera", data.resumen.byEstado.enEspera],
      ["En progreso", data.resumen.byEstado.enProgreso],
      ["Terminadas", data.resumen.byEstado.terminado],
      ["% Completadas", `${data.resumen.porcentajeCompletadas}%`],
      [],
      ["TIEMPOS PROMEDIO"],
      ["Primer contacto (días)", data.resumen.tiempoPromedioRespuesta ?? "N/A"],
      ["Cierre (días)", data.resumen.tiempoPromedioCierre ?? "N/A"],
      [],
    ...
    const detalleHeaders = [
      "N°", "Medio", "Fecha Recibido", "Mes", "Bloque", "Apto",
      "Nombre Residente", "Tipo", "Asunto", "Descripción",
      "Fecha Primer Contacto", "Tiempo Resp. Primer Contacto",
      "Acción Tomada", "Estado", "Evidencia Cierre",
      "Fecha Cierre", "Tiempo Resp. Cierre", "Gestionado Por",
    ];

    const detalleRows = data.detalle.map((d) => [
      d.numero, d.medio, d.fechaRecibido, d.mes, d.bloque, d.apto,
      d.nombreResidente, d.tipoPqrs, d.asunto, d.descripcion,
      d.fechaPrimerContacto, d.tiempoRespuestaPrimerContacto,
      d.accionTomada, d.estado, d.evidenciaCierre,
      d.fechaCierre, d.tiempoRespuestaCierre, d.gestionadoPor,
    ]);
    ...
    const ws2 = XLSX.utils.aoa_to_sheet([detalleHeaders, ...detalleRows]);
    // Auto-width columns
    ws2["!cols"] = detalleHeaders.map(() => ({ wch: 18 }));
    XLSX.utils.book_append_sheet(wb, ws2, "Seguimiento PQRS");

    const filename = `PQRS_${month ? MESES[parseInt(month) - 1] + "_" : ""}${year}.xlsx`;
